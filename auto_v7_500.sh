...
tg "[EW Coach] v7 starting: 500 INBOUND sims, Haiku operator, 5 workers."
...
/opt/expwholesale/venv/bin/python3 /opt/expwholesale/sim_inbound.py \
    --runs 100 --workers 5 \
    --out $LOGDIR/sim_results_v7.json \
    > $LOGDIR/sim_run_v7.log 2>&1

T_END=$(date +%s)
DUR=$((T_END - T_START))
N_OK=$(/opt/expwholesale/venv/bin/python3 -c "import json; print(len(json.load(open('/opt/expwholesale/logs/sim_results_v7.json'))))" 2>/dev/null || echo "?")

tg "[EW Coach] v7 sims DONE in ${DUR}s. ${N_OK}/500 succeeded. Running single Opus Coach pass on 30 samples..."

if [ "$N_OK" = "0" ] || [ "$N_OK" = "?" ]; then
  tg "[EW Coach] v7 produced no transcripts. Check /opt/expwholesale/logs/sim_run_v7.log."
  exit 1
fi
...
COACH_BYTES=0
tg "[EW Coach] v7 FINAL: ${N_OK}/500 transcripts, total wall=${TOTAL}s. ${SCORES}. Coach: ${COACH_BYTES}B at /opt/expwholesale/logs/sim_results_v7_coach.md"
