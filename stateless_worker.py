...
# Vendor-specific preferred (disposition, condition) tuples — matches the
# OLD Selenium worker's display values. Falls back to the first pricing if
# nothing matches.
_PREFERRED = {
...
def extract_book_value(book, book_name: str = None):
    """Return final price (base + odometer adjustment + historical adjustment)
    for the dealer-preferred (disposition, condition) — matches vAuto UI."""
    ...
    def _total(pricing):
        try:
            base = float(pricing.get('basePrice') or 0)
            odo = float(pricing.get('odometerAdjustment') or 0)
            hist = float(pricing.get('historicalAdjustment') or 0)
            return int(round(base + odo + hist))
        except (ValueError, TypeError):
            return None

    pref_list = _PREFERRED.get(book_name) if book_name else []
    for disp, cond in pref_list:
        for pricing in pricings:
            if pricing.get('disposition') == disp and pricing.get('condition') == cond:
                v = _total(pricing)
                if v is not None and v > 0:
                    return v
    # Fallback: first pricing with non-zero base
    for pricing in pricings:
        v = _total(pricing)
        if v is not None and v > 0:
            return v
...
def upsert_lookup(conn, bid_id, vin, appraisal_id, vehicle, books, carfax_url):
    ...
    rbook = extract_book_value(books.get('rBook') if books else None, 'rBook')  # likely null
    black_book = extract_book_value(books.get('blackBook') if books else None, 'blackBook')
    kbb = extract_book_value(books.get('kbb') if books else None, 'kbb')
    kbb_com = extract_book_value(books.get('kbbOnline') if books else None, 'kbbOnline')
    nada = extract_book_value(books.get('nada') if books else None, 'nada')
    manheim = extract_book_value(books.get('manheim') if books else None, 'manheim')
    jd_power = extract_book_value(books.get('jdPower') if books else None, 'jdPower')  # likely null
