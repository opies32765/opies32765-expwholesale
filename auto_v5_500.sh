...
tg "[EW Coach] v5 starting: 500 sims, Haiku partner, 5 workers, NO Coach per batch."
...
/opt/expwholesale/venv/bin/python3 /opt/expwholesale/sim_outbound.py \
    ...
    --runs 100 \
    --workers 5 \
    --out $LOGDIR/sim_results_v5.json \
    > $LOGDIR/sim_run_v5.log 2>&1

V5_EXIT=$?
T_V5=$(date +%s)
DUR=$((T_V5 - T_START))
N_OK=$(/opt/expwholesale/venv/bin/python3 -c "import json; print(len(json.load(open(\"/opt/expwholesale/logs/sim_results_v5.json\"))))" 2>/dev/null || echo "?")

tg "[EW Coach] v5 sims DONE in ${DUR}s. ${N_OK}/500 succeeded. Now running single Opus Coach pass on 30-transcript sample..."

if [ "$N_OK" = "0" ] || [ "$N_OK" = "?" ]; then
  tg "[EW Coach] v5 produced no transcripts. Check /opt/expwholesale/logs/sim_run_v5.log."
  exit 1
fi
...
sims = json.load(open("/opt/expwholesale/logs/sim_results_v5.json"))
...
open("/opt/expwholesale/logs/sim_results_v5_coach.md","w").write(out)
...
if scores_match:
    ...
    summary = f"v5 scores: Nat={nat} Acc={acc} Per={per} Rec={rec} Close={clo}"
    print(summary)
    open("/tmp/v5_scores.txt","w").write(summary)
PYEOF

T_END=$(date +%s)
TOTAL=$((T_END - T_START))
SCORES=$(cat /tmp/v5_scores.txt 2>/dev/null || echo "scores not parsed - see /opt/expwholesale/logs/sim_results_v5_coach.md")
COACH_BYTES=$(wc -c < /opt/expwholesale/logs/sim_results_v5_coach.md 2>/dev/null || echo "?")

tg "[EW Coach] FINAL: ${N_OK}/500 transcripts, total wall=${TOTAL}s. ${SCORES}. Coach report: ${COACH_BYTES} bytes. Path: /opt/expwholesale/logs/sim_results_v5_coach.md"
